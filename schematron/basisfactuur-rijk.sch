...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
  ...
  <!--
    ...
    Datum schematron         - 10 maart 2023
  ...
  <pattern id="type-restrictions">
    ...
    <rule context="doc:Invoice/cac:OrderReference">
      <assert test="string-length(cbc:ID) > 0">BR-NL-BFR-7: Het veld 'cac:OrderReference/cbc:ID' is leeg of bestaat niet. Gebruik het veld 'cac:OrderReference/cbc:ID' voor 'Basisfactuur 1.5 Ordernummer / referentie' (verplicht).</assert>
    </rule>
    <rule context="doc:Invoice/cac:AccountingSupplierParty/cac:Party/cac:PartyTaxScheme">
      <assert test="string-length(cbc:CompanyID) > 0">BR-NL-BFR-16: Het veld 'doc:Invoice/cac:AccountingSupplierParty/cac:Party/cac:PartyTaxScheme/cbc:CompanyID' is leeg of bestaat niet. Vul dit veld met de inhoud van 'Basisfactuur 2.3 - BTW-nummer' (verplicht).</assert>
    </rule>
    ...
    <rule context="doc:Invoice/cac:TaxTotal/cac:TaxSubtotal">
      <assert test="cbc:TaxableAmount[@currencyID eq //doc:Invoice/cbc:DocumentCurrencyCode]">BR-NL-BFR-35: Het attribuut 'currencyID' van het element 'doc:Invoice/cac:TaxTotal/cac:TaxSubtotal/cbc:TaxableAmount' moet gelijk zijn aan de waarde van 'doc:Invoice/cbc:DocumentCurrencyCode' (Basisfactuur 1.4 valuta).</assert>
    </rule>
  </pattern>
</schema>
